...
class VSSBlock(nn.Module):
    def __init__(
        ...
        if self.use_modulation:
            self.condition = nn.Parameter(torch.randn(self.condition_size, 128), requires_grad=True)
    ...
    def forward(self, input: torch.Tensor, condition: torch.Tensor):
        ...
        if self.use_modulation:
            ...
            if condition.dim() == 1:
                condition = F.one_hot(condition, num_classes=self.condition_size).float()
            indices = torch.argmax(condition, dim=1) # previous settings
            #indices = torch.argmax(condition, dim=0)
...
class GraphNeuralNetwork(nn.Module):
    ...
    def forward(self, data):
        # data contains node features, edge index, and edge attributes (optional)
...
class VSSM(nn.Module):
    ...
    def forward_features_up(self, x, skip_list, condition):
        # Ensure condition is passed to upsample layers
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = layer_up(x, condition)
            else:
                x = layer_up(x + skip_list[-inx], condition)  # Pass condition here
        return x
    ...
    def forward(self, x, condition):
        x, skip_list = self.forward_features(x, condition)
        x = self.forward_features_up(x, skip_list, condition)
        x = self.forward_final(x)

        return x

# Example VSSLayer with Graph Conditioning
class VSSLayer_graph(nn.Module):
    def __init__(self, dim, depth, d_state, drop, attn_drop, drop_path, norm_layer, downsample=None, use_checkpoint=False):
        super(VSSLayer_graph, self).__init__()
        # Layer initialization (unchanged)

    def forward(self, x, condition, graph_embedding):
        # Original state transition example
        h_t_next = self.state_transition(x, condition)  # State update from condition
        
        # Incorporate graph embedding into the state transition
        h_t_next = h_t_next + graph_embedding.unsqueeze(1)  # Fuse graph embedding
        
        # Return updated state
        return h_t_next

# Modified VSSM architecture
class VSSM_graph(nn.Module):
    def __init__(self, patch_size=4, in_chans=3, num_classes=1000, depths=[2, 2, 9, 2], depths_decoder=[2, 9, 2, 2],
        ...
        # Patch embedding and position embedding setup
        self.patch_embed = PatchEmbed2D(patch_size=patch_size, in_chans=in_chans, embed_dim=self.embed_dim,
                                        norm_layer=norm_layer if patch_norm else None)
        self.pos_drop = nn.Dropout(p=drop_rate)
        ...
        # Graph Embedding Network (GNN)
        self.gnn = GCNConv(in_channels=dims[0], hidden_dim=dims[1], out_channels=dims[0])  # Example GCN layer for graph embedding

    def forward_features(self, x, condition, graph_data):
        skip_list = []
        ...
        # Step 2: Process the graph data with the GNN
        graph_embedding = self.gnn(graph_data.x, graph_data.edge_index)
        
        # Step 3: Process layers with graph-conditioned state transitions
        for layer in self.layers:
            skip_list.append(x)
            x = layer(x, condition, graph_embedding)  # Pass graph embeddings to each layer

        return x, skip_list

    def forward_features_up(self, x, skip_list, condition, graph_embedding):
        # Process the upsampled features, ensuring graph data is used
        for inx, layer_up in enumerate(self.layers_up):
            if inx == 0:
                x = layer_up(x, condition)
            else:
                x = layer_up(x + skip_list[-inx], condition)
        return x
    ...
    def forward(self, x, condition, graph_data):
        x, skip_list = self.forward_features(x, condition, graph_data)
        x = self.forward_features_up(x, skip_list, condition, graph_data)
        x = self.forward_final(x)
        return x